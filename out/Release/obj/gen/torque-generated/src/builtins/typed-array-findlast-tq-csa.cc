...
TNode<Object> FindLastAllElements_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSTypedArray> p_array, TNode<JSReceiver> p_predicate, TNode<Object> p_thisArg) {
  compiler::CodeAssembler ca_(state_);
...
tmp14 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallableFor(ca_.isolate(),ExampleBuiltinForTorqueFunctionPointerType(1)).descriptor(), tmp2, TNode<Object>(), tmp13, tmp6));
  ...
  TNode<Object> phi_bb17_4;
    ca_.Bind(&block17, &phi_bb17_4);
  return TNode<Object>{phi_bb17_4};
}
...
TF_BUILTIN(TypedArrayPrototypeFindLast, CodeStubAssembler) {
  ...
  if (block11.is_used()) {
    ...
    tmp12 = FindLastAllElements_0(state_, TNode<Context>{parameter0}, TNode<JSTypedArray>{tmp2}, TNode<JSReceiver>{tmp6}, TNode<Object>{tmp11});
