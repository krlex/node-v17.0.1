...
TNode<Object> FindAllElements_0(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSTypedArray> p_array, TNode<JSReceiver> p_predicate, TNode<Object> p_thisArg) {
  compiler::CodeAssembler ca_(state_);
...
tmp12 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallableFor(ca_.isolate(),ExampleBuiltinForTorqueFunctionPointerType(1)).descriptor(), tmp2, TNode<Object>(), tmp11, phi_bb13_8));
  ...
  if (block7.is_used()) {
    ...
    tmp14 = CodeStubAssembler(state_).Call(TNode<Context>{p_context}, TNode<Object>{p_predicate}, TNode<Object>{p_thisArg}, TNode<Object>{phi_bb7_9}, TNode<Object>{tmp13}, TNode<Object>{tmp0});
    tmp15 = ToBoolean_0(state_, TNode<Object>{tmp14});
    ca_.Branch(tmp15, &block16, std::vector<compiler::Node*>{phi_bb7_8}, &block17, std::vector<compiler::Node*>{phi_bb7_8});
  }

  TNode<UintPtrT> phi_bb16_8;
  if (block16.is_used()) {
    ca_.Bind(&block16, &phi_bb16_8);
    ca_.Goto(&block1, phi_bb7_9);
  }
  ...
  if (block17.is_used()) {
    ...
    tmp17 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb17_8}, TNode<UintPtrT>{tmp16});
    ca_.Goto(&block5, phi_bb7_5, tmp17);
  }
  ...
  return TNode<Object>{phi_bb18_4};
}
